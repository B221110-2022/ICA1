...
Read_pairs_num=100000

## recursively copy experimental RNAseq data in fastq folder to current directory.
#cp -r /localdisk/data/BPSM/ICA1/fastq .


########### Code block 1 : Integrity check ########
#echo "Quick check for integrity is processing..."
#rm -f integrity_check.txt  # in case if the output file was already there
...
#fi
#done 
#defeat=$(wc -l integrity_check.txt | cut -d ' ' -f1) 
#if [ ${defeat} == 0 ];then
...
#fastqc fastq/*fq.gz -o report --extract  # output to dir report.
